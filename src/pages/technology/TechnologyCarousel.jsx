...
const TechnologyImage = ({ item }) => {
  return (
    <div className="img-container flex justify-center w-full h-full  md:h-[310px] lg:w-fit lg:h-[712px]">
      <picture>
...
const SlideIndicator = ({item, index, onClick, isActive}) => {
  return <div className={`${isActive ? ' bg-white' : 'bg-transparent text-white'} h-10 w-10 md:h-[60px] md:w-[60px] md:mt-14 rounded-full text-center leading-[40px] md:leading-[60px] box-border border-gray-600 border-[1px] inline-block hover:bg-white hover:text-black cursor-pointer`} onClick={() => onClick(item)}>{index+1}</div>
}
...
const InfoContainer = ({ item }) => {
  return (
    <div className="info-container flex flex-col items-center justify-between h-full px-6 md:px-0 md:w-[458px] md:m-auto lg:items-start lg:m-0 lg:px-0 lg:w-full">
      ...
      <h3 className="font-bellefair text-md md:text-[40px] lg:text-xl pt-2.5 md:py-4 uppercase text-white">{item.name}</h3>
...
const TechnologyCarousel = ({items}) => {
  return (
    <Carousel
          ...
          slideIndicatorClassName="flex flex-row items-center w-[152px] md:w-[210px] justify-between h-fit-content mt-8 md:mt-0 lg:self-baseline lg:col-start-3 lg:col-end-7 lg:row-start-6 lg:row-end-6 lg:m-0 "
          ...
          bottomContainerClassName="bottom-container mt-8 md:mt-11 lg:col-start-3 lg:col-end-12 lg:row-start-2 lg:row-end-6 lg:m-0"
